...
function renderShape(element) {
  ...
  if (shape === 'triangle') {
    ...
    const hasStroke = element.stroke && element.stroke !== 'none' && element.stroke !== 'transparent';
    const stroke = hasStroke ? element.stroke : 'none';
    const strokeWidth = hasStroke ? element.stroke_width ?? 1 : 0;
    return `<svg aria-hidden="true" viewBox="0 0 100 100" preserveAspectRatio="none" style="${baseBoxStyle(element)} ${style({ overflow: 'visible' })}"><polygon points="50,0 0,100 100,100" fill="${escapeHtml(fill)}" stroke="${escapeHtml(stroke)}" stroke-width="${escapeHtml(strokeWidth)}" vector-effect="non-scaling-stroke" stroke-linejoin="round"></polygon></svg>`;
  }
...
// does not retain filesystem dependencies. HTTP(S), protocol-relative URLs,
// and data URIs are preserved.
