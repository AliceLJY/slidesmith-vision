...
// Local images are inlined as data URIs so the generated HTML stays
// self-contained — SlideSmith's converter serves only the HTML's own
// directory, so filesystem paths would 404 there.
function resolveImageSrc(src, baseDir) {
  if (!src) return '';
  if (/^(https?:|data:)/.test(src)) return src;
  const filePath = src.startsWith('file:')
    ? fileURLToPath(src)
    : (path.isAbsolute(src) ? src : path.resolve(baseDir, src));
  try {
    const mime = IMAGE_MIME[path.extname(filePath).toLowerCase()] || 'application/octet-stream';
    return `data:${mime};base64,${readFileSync(filePath).toString('base64')}`;
  } catch {
    console.warn(`slidesmith-vision: cannot read image ${filePath}, leaving src as-is`);
    return src;
  }
}
...
function px(value) {
  ...
  if (!Number.isFinite(n)) {
    console.warn(`slidesmith-vision: non-numeric size value ${JSON.stringify(value)}, using 0`);
    return '0px';
  }
  return `${n}px`;
}
